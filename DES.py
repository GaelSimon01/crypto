...
mc1=encrypt(cle1,message)
mc2=encrypt(cle2,mc1)

mdc1=decrypt(cle2,mc2)
mdc2=decrypt(cle1,mdc1)



def methode_brut(message_crypter,taille_cle1,taille_cle2):
    for i in range(2**taille_cle1):
        for j in range(2**taille_cle2):
            mdc1=decrypt(i,message_crypter)
            mdc2=decrypt(j,mdc1)
            if mdc2==message:
                return (i,j)
    return (0,0)

print(methode_brut(mc2,10,10))
